import type { BuilderIdeaInput, ProjectSpec, TypeDetectionResult } from "@combi/shared";
import { getMode } from "../../shared/modeRegistry";

export class CreateProjectSpecStep {
  execute(input: BuilderIdeaInput, detection: TypeDetectionResult): ProjectSpec {
...
function createProjectName(prompt: string, kind: ProjectSpec["kind"]): string {
  const words = prompt
    .trim()
    .split(/\s+/)
    .slice(0, 4)
